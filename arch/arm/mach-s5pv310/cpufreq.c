...
enum cpufreq_level_index{
	L0, L1, L2, L3, L4, L5, L6, L7, L8, L9, L10, CPUFREQ_LEVEL_END,
};
#else
enum cpufreq_level_index{
...
void s5pv310_set_frequency(unsigned int old_index, unsigned int new_index)
{
	...
	if (freqs.old < freqs.new) {
		...
		if (s5pv310_max_armclk == ARMCLOCK_1600MHZ) {
			/* L6/L8, L7/L9 Level change require to only change s value */
			if (is_curfreq_table &&
				(((old_index == L8) && (new_index == L5)) ||
				((old_index == L9) && (new_index == L6)) ||
				((old_index == L10) && (new_index == L7))))
					change_s_value = 1; 

	        } else {
			/* L5/L7, L6/L8 Level change require to only change s value */
			if (is_curfreq_table &&
				(((old_index == L6) && (new_index == L3)) ||
				((old_index == L7) && (new_index == L4)) ||
				((old_index == L8) && (new_index == L5))))
					change_s_value = 1;
		}
		...
		if (s5pv310_max_armclk == ARMCLOCK_1600MHZ) {
			/* L6/L8, L7/L9 Level change require to only change s value */
			if (is_curfreq_table &&
				(((old_index == L5) && (new_index == L8)) ||
				((old_index == L6) && (new_index == L9)) ||
				((old_index == L7) && (new_index == L10))))
					change_s_value = 1;

		} else {
			/* L4/L6, L5/L7 Level change require to only change s value */
			if (is_curfreq_table &&
				(((old_index == L3) && (new_index == L6)) ||
				((old_index == L4) && (new_index == L7)) ||
				((old_index == L5) && (new_index == L8))))
					change_s_value = 1;
		}
		...
		unsigned long event, void *ptr)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		s5pv310_freq_table[L7].frequency, DISABLE_FURTHER_CPUFREQ);
		if (WARN_ON(ret < 0))
			return NOTIFY_BAD;
#ifdef CONFIG_S5PV310_BUSFREQ
	...
	case PM_POST_SUSPEND:
		...
		ret = cpufreq_driver_target(policy,
		s5pv310_freq_table[L7].frequency, ENABLE_FURTHER_CPUFREQ);
		policy->max = max;
		policy->min = min;
#ifdef CONFIG_S5PV310_BUSFREQ
